...
function M.connect_to_redis ()
    ...
    -- hack added to handle different names for redis service in different environments.
    --  assumes that the last redis service bound to this app is the session store.  -gstroup
    if (redis_key ==nil) then
        for k,v in pairs(vcap_services) do
            if string.match(k, 'redis') then
                ngx.log(ngx.ERR, 'REDIS KEY: '..k)
                redis_key = k
            end
        end
    end
    ...
    local redis_credentials = vcap_services[redis_key][1].credentials
