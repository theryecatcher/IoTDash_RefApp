...
public class GroupFactoryIT extends BaseFactoryIT
{
    ...
    private static final Logger log = LoggerFactory.getLogger(GroupFactoryIT.class);
    ...
    public void testCreateGroupsOfLevelEnterprise(List<Header> headers)
    {
        ...
        list.add(group);
        list.add(group2);
        CloseableHttpResponse response = this.groupFactory.create(list, headers);
    }
    ...
    {
        List<Group> list = this.groupFactory.getGroupsByFilter(null, "level", "enterprise", headers);//$NON-NLS-1$//$NON-NLS-2$
    ...
    {
        Group group = this.groupFactory.getGroup("AirProds_SA_NVEnterprise", headers);//$NON-NLS-1$
        ...
        assertEquals("Air Products SA/NV enterprise -- updated", group.getDescription());//$NON-NLS-1$
    ...
    {
        HttpResponse response = this.groupFactory.deleteGroup("AirProds_SA_NVEnterprise", headers);//$NON-NLS-1$
        Assert.assertEquals(response.getStatusLine().getStatusCode(),HttpStatus.SC_NO_CONTENT);

        response = this.groupFactory.deleteGroup("Suncore_EnergieEnterprise", headers); //$NON-NLS-1$
        Assert.assertEquals(response.getStatusLine().getStatusCode(),HttpStatus.SC_NO_CONTENT);
    ...
    {
        List<Group> list = this.groupFactory.getGroupsByFilter(null, "level", "site", headers);//$NON-NLS-1$//$NON-NLS-2$
        assertTrue(list.size() > 0);
    }
    ...
    public void testUpdateGroupOfLevelSite(List<Header> headers)
    {
        ...
        assertEquals("Site: Gent, Belgium desc -- updated", group.getDescription()); //$NON-NLS-1$
    ...
    {
        HttpResponse response = this.groupFactory.deleteGroup("montreal_qc_canada", headers); //$NON-NLS-1$
        Assert.assertEquals(response.getStatusLine().getStatusCode(),HttpStatus.SC_NO_CONTENT);
