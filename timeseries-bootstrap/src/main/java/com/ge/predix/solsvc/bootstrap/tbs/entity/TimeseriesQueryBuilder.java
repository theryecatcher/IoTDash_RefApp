...
public class TimeseriesQueryBuilder {
	...
	static class TimeSeriesQueryTagOrderSerializer implements
			JsonSerializer<Order> {

		@Override
		public JsonElement serialize(Order order, Type arg1,
	...
	private TimeseriesQueryBuilder() {
		...
		builder.registerTypeAdapter(CustomAggregator.class,
				new CustomAggregatorSerializer());
		builder.registerTypeAdapter(ListMultimap.class,
				new ListMultiMapSerializer());
		builder.registerTypeAdapter(Order.class,
				new TimeSeriesQueryTagOrderSerializer());
	...
	public TimeseriesQueryBuilder setStart(Date start) {
		checkNotNull(start);
		checkArgument(startRelative == null,
				"Both relative and absolute start times cannot be set.");

		this.startAbsolute = start.getTime();
		checkArgument(startAbsolute <= System.currentTimeMillis(),
				"Start time cannot be in the future.");
		return this;
	}

	/**
	 * The beginning time of the time range relative to now. For example, return
	 * all data points that starting 2 days ago.
	 *
	 * @param duration
	 *            relative time value
	 * @param unit
	...
	public TimeseriesQueryBuilder setStart(int duration, TimeUnit unit) {
		...
		checkNotNull(unit);
		checkArgument(startAbsolute == null,
				"Both relative and absolute start times cannot be set.");

		startRelative = new RelativeTime(duration, unit);
		checkArgument(startRelative.getTimeRelativeTo(System
	...
	public TimeseriesQueryBuilder setEnd(Date end) {
		checkArgument(endRelative == null,
				"Both relative and absolute end times cannot be set.");
	...
	public TimeseriesQueryBuilder setEnd(int duration, TimeUnit unit) {
		checkNotNull(unit);
		checkArgument(duration > 0);
		checkArgument(endAbsolute == null,
				"Both relative and absolute end times cannot be set.");
	 ...
	 * be used by the client to query KairosDB
	 *
	 * @return JSON
	 * @throws IOException
	 *             if the query is invalid and cannot be converted to JSON
	 */
	...
	private void validateTimes() {
		checkState(startAbsolute != null || startRelative != null,
				"Start time must be specified");

		if (endAbsolute != null) {
			if (startAbsolute != null)
				TimeValidator.validateEndTimeLaterThanStartTime(startAbsolute,
						endAbsolute);
			else
				TimeValidator.validateEndTimeLaterThanStartTime(startRelative,
						endAbsolute);
