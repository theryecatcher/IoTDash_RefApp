...
def createPredixCloudIdentityForMachine():
	...
	shutil.copy("scripts/com.ge.dspmicro.predixcloud.identity.cfg.template", "scripts/com.ge.dspmicro.predixcloud.identity.cfg")
	s = open("scripts/com.ge.dspmicro.predixcloud.identity.cfg").read()
	s = s.replace('${UAAUrl}', uaaIssuerId)
	s = s.replace('${clientId}', config.rmdAppClientId)
	s = s.replace('${clientSecret} ', config.rmdAppSecret)
	
	f = open("scripts/com.ge.dspmicro.predixcloud.identity.cfg", 'w')
	f.write(s)
	f.close()
	with open('scripts/com.ge.dspmicro.predixcloud.identity.cfg', 'r') as fin:
		print (fin.read())
#######################################
# Begin Main script
#######################################
